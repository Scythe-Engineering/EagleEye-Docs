...
Once EagleEye is running, use the web UI to join or change Wi-Fi networks:

1. Open **Settings** from the navigation on the right.
...
The active network has a green **Connected** label. Its action button changes to **Disconnect**.
Keep Ethernet connected until the Wi-Fi row shows **Connected**, especially if the browser is
using the Ethernet address.

## Connect over SSH

Use SSH when EagleEye is not installed yet, or when the web UI is unavailable. If entering Wi-Fi
details in Raspberry Pi Imager is inconvenient, use the robot network for the first SSH
connection, then join an internet-connected Wi-Fi network or phone hotspot from the Pi.
...
**Expected result:** you get a shell prompt on the Pi while staying connected through Ethernet.
...
If it fails because the robot network remains the preferred internet route, give Wi-Fi a lower
route metric:
...
You can also note the `wlan0` address, move your laptop onto the same hotspot, unplug Ethernet,
and reconnect with `ssh <username>@<wifi-ip>`. Some phone hotspots isolate connected devices; in
that case, keep managing the Pi over Ethernet and use the route-metric method above.
